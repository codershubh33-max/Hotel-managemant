...
-- 5. Display bookings with customer names
SELECT
    b.booking_id,
    c.first_name,
    c.last_name,
...
-- 6. Display room details with room type
SELECT
    r.room_number,
    rt.type_name,
    rt.price_per_night,
...
-- 7. Display hotel name with room number
SELECT
    h.hotel_name,
    r.room_number,
    r.status
...
-- Highest room price
SELECT MAX(price_per_night) AS Highest_Price
FROM RoomType;
